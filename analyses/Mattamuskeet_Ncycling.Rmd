---
title: "Wetland Conditions Differentially Influence Nitrogen Processing Within Waterfowl Impoundments"
...
p1a <- ggplot(master, aes(x = as.factor(time), y = denitrification, fill = group)) +
  ...
  scale_colour_manual(name="Site", values=c("gold3","deepskyblue3"))+
  scale_fill_manual(name="Site", values=c("gold3","deepskyblue3"))+ labs(x = "", y = (expression(paste("Denitrification (ng N"[2],"O (g DM)"^-{1}," hr"^-{1},")")))) 
p1<- p1a + theme(panel.grid.major = element_blank(), 
...
p2a <- ggplot(master, aes(x = as.factor(time), y = nitrification, fill = group)) +
  geom_flat_violin(aes(fill = group),position = position_nudge(x = .1, y = 0), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA)+
  ...
  scale_colour_manual(name="Site", values=c("gold3","deepskyblue3"))+
  scale_fill_manual(name="Site", values=c("gold3","deepskyblue3"))+ labs(x = "", y = (expression(paste("Nitrification (ng NO"[2]," (g DM)"^-{1}," hr"^-{1},")"))))
p2<- p2a + theme(panel.grid.major = element_blank(), 
               panel.grid.minor = element_blank(), 
               axis.line = element_line(colour = "black")) + 
              theme(panel.background = element_blank())+
theme(axis.title = element_text(size=14), axis.text = element_text(size=14),
...
p3a <- ggplot(master, aes(x = as.factor(time), y = Nmineralization_ng, fill = group)) +
  geom_flat_violin(aes(fill = group),position = position_nudge(x = .1, y = 0), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA)+
  ...
  scale_colour_manual(name="Site", values=c("gold3","deepskyblue3"))+
  scale_fill_manual(name="Site", values=c("gold3","deepskyblue3"))+ labs(x = "Time Point", y=expression(paste("N Mineralization (ng ",NH[4]^{'+'},"-N (g DM)"^-{1}," d"^-{1},")"))) + scale_x_discrete(labels=c("1" = "July", "2" = "September", "3" = "December"))
p3<- p3a + theme(panel.grid.major = element_blank(), 
               panel.grid.minor = element_blank(), 
               axis.line = element_line(colour = "black")) + 
              theme(panel.background = element_blank())+
theme(axis.title = element_text(size=14), axis.text = element_text(size=14),
...
p1a <-p1 + theme(legend.position="top", legend.box = "horizontal")+   theme(legend.text=element_text(size=14),legend.title=element_text(size=14)) + rremove("x.text")
...
ggsave("../figures/Figure2_rainclouds.png", plot=last_plot(), device=NULL, path=NULL, scale=1,  width=7, height=12, dpi=300, limitsize=TRUE)
```
#graph nitrification x moisture and pH
```{r Loading Files, include=FALSE}
p <- ggplot(master, aes(x=Moisture, y=nitrification, shape=as.factor(Month), color=as.factor(Site)))+scale_shape_manual(name="Month", values=c(15,16,17), labels=c("July","September","December"))+scale_color_manual(name="Site", values=c("gold3","deepskyblue3"), labels = c("Ag", "MSM")) +  geom_point(size=4) + scale_x_continuous(limits=c(0,60))+scale_y_continuous(
...
p <- ggplot(master, aes(x=pH, y=nitrification, shape=as.factor(Month), color=as.factor(Site)))+scale_shape_manual(name="Month", values=c(15,16,17), labels=c("July","September","December"))+scale_color_manual(name="Site", values=c("gold3","deepskyblue3"), labels = c("Ag", "MSM")) +  geom_point(size=2) + scale_x_continuous(limits=c(4,6.5))+scale_y_continuous(labels = scales::number_format(accuracy = 0.1))
#p1=p+geom_smooth(method="lm")
...
```{r Loading Files, include=FALSE}
p <- ggplot(master, aes(x=Moisture, y=denitrification, shape=as.factor(Month), color=as.factor(Site)))+scale_shape_manual(name="Month", values=c(15,16,17), labels=c("July","September","December"))+scale_color_manual(name="Site", values=c("gold3","deepskyblue3"), labels = c("Ag", "MSM")) +  geom_point(size=4) + scale_x_continuous(limits=c(0,60))
p1=p+geom_smooth(method="lm")
...
```{r Loading Files, include=FALSE}
p <- ggplot(master, aes(x=Moisture, y=Nmineralization_ng, shape=as.factor(Month), color=as.factor(Site)))+scale_shape_manual(name="Month", values=c(15,16,17), labels=c("July","September","December"))+scale_color_manual(name="Site", values=c("gold3","deepskyblue3"), labels = c("Ag", "MSM")) +  geom_point(size=4) + scale_x_continuous(limits=c(0,60))
p1=p+geom_smooth(method="lm")
...
# remove legends and axes
n1 <-denitrif + theme(legend.position="top", legend.box = "horizontal")+ rremove("x.text")+   theme(legend.text=element_text(size=14),legend.title=element_text(size=14))
...
ggsave("../figures/Figure3_Nrates_Moisture.png", plot=last_plot(), device=NULL, path=NULL, scale=1,  width=8, height=15, dpi=300, limitsize=TRUE)

```
# Discussion - additional stats
```{r Loading Files, include=FALSE}
#pH for MSM (and incorporating wet vs. dry)
pH.MSM.lm <- lm(pH ~ Hydrology*Month, data = master.MSM)
summary(pH.MSM.lm, ddf="Kenward-Roger")

#graph
p1 <-ggplot(master.MSM, aes(x = factor(Month), y = pH, fill=Hydrology)) +
      theme_bw() + 
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line =    
            element_line(colour = "black")) 
p1.pH <- p1 + geom_boxplot(alpha = 0.90) + scale_fill_manual(values=c("white", "skyblue3")) +
      geom_point(aes(fill = factor(Hydrology)), size=2, shape=21, position = position_jitterdodge()) +
      labs(x="Time Point", y="Soil pH") + scale_x_discrete("", labels = c("1.July" = "July","2.September" = 
      "September", "3.December" = "December")) + 
...
#graph
p2 <-ggplot(master.Ag, aes(x = factor(Month), y = pH, fill=Hydrology)) +
      theme_bw() + 
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line =    
            element_line(colour = "black")) 
p2.pH <- p2 + geom_boxplot(alpha = 0.90) + scale_fill_manual(values=c("white", "skyblue3")) +
      geom_point(aes(fill = factor(Hydrology)), size=2, shape=21, position = position_jitterdodge()) +
      labs(x="Month", y="Soil pH") + 
  scale_x_discrete("Time Point", labels = c("1.July" = "July","2.September" = 
      "September", "3.December" = "December")) + 
  theme(axis.title=element_text(vjust=1,size=16), 
        axis.text=element_text(size=16), 
        axis.text.x = element_text(vjust=0.65, hjust=0.5, size=16), 
...
# remove legends and axes
n1 <-p1.pH + theme(legend.position="top", legend.box = "horizontal")+ rremove("x.text")+ theme(legend.text=element_text(size=14))
n2 <- p2.pH + theme(legend.position="none")
...
ggsave("../figures/FigureS1_hydrology.png", plot=last_plot(), device=NULL, path=NULL, scale=1,  width=6, height=8, dpi=300, limitsize=TRUE)
```
...
ggsave("../figures/FigS2_mesoN.png", plot=last_plot(), device=NULL, path=NULL, scale=1,  width=9, height=12, dpi=300, limitsize=TRUE)
